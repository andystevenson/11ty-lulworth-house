...
const getSiblingContent = (collection, item, limit = 3, random = true) => {
...
const filterCollectionByKeys = (collection, keys) => {
  return collection.filter(x => keys.includes(x.data.key));
}

const images = (list, path) => {
  return list.filter(f => f.includes(path)).sort().map(i => i.replace('./src', '..'));
}
